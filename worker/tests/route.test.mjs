...
test('crossing: nearest neighbour crosses itself, the built route does not, and the order is exactly 2, 3, 5, 4, 1', () => {
  ...
  assert.deepEqual(nn.map(s => s.client_id), [2, 4, 1, 3, 5])
  ...
  assert.deepEqual(built.map(s => s.client_id), [2, 3, 5, 4, 1])
})
...
test('deterministic: same input, same order, whatever order the input came in', () => {
  for (const stops of INSTANCES.slice(0, 10)) {
    ...
    assert.deepEqual(orderStops(YARD, stops).map(s => s.client_id), once)
    assert.deepEqual(orderStops(YARD, [...stops].reverse()).map(s => s.client_id), once)
  }
})
...
test('nearest neighbour ties go to the lower client_id', () => {
  ...
  assert.deepEqual(nearestNeighbour(YARD, [east, west]).map(s => s.client_id), [4, 9])
})
...
test('truck assignment: own truck kept, orphan to the truck with the nearest stop, tie to fewer stops then lower id', () => {
  ...
  const clients = [at(1, 0.010, 0, 2), at(2, -0.010, 0, 2), at(3, -0.0101, 0, null), at(4, 0.0101, 0.0001, 7), at(5, 0.0099, 0, 1)]
  ...
  assert.deepEqual(tie.map(t => [t.truck_id, t.stops.map(s => s.client_id)]), [[1, [1]], [2, []]])
  ...
  assert.deepEqual(eq.map(t => t.stops.map(s => s.client_id).sort()), [[1, 2], [3, 4]])
})
...
test('buildRoute orders each truck from the yard with medical first', () => {
  ...
  for (const t of buildRoute(YARD, clients, [1, 2])) {
    assert.deepEqual(t.stops.map(s => s.client_id), orderStops(YARD, clients.filter(c => c.truck_id === t.truck_id)).map(s => s.client_id))
  }
})
