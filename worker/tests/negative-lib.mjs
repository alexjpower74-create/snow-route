...
export function createLog(title) {
  ...
  const dirty = spawnSync('git', ['status', '--porcelain', '--', '.', ':!tests/negative-control.log'], {
    cwd: root,
    encoding: 'utf8',
  }).stdout.trim()
    ? '+uncommitted worker changes'
    : ''
...
export async function runControl({ title, testFile, tests, breaks, setup = [], check, describe, api = false }) {
  ...
  try {
    ...
    log.say(
      `control (unbroken copy): ${tests.map((t) => `"${t}"`).join(', ')} ${cleanOk ? 'pass' : `DID NOT PASS (exit ${before.status})\n${before.output}`}`,
    )
    if (worker) {
      await worker.stop()
      worker = null
    ...
    log.say(
      ok
        ? `verdict: RED as expected. ${tests.map((t) => `"${t}"`).join(' and ')} fail when ${describe}.`
