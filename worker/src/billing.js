...
export const SEASONAL_NOTE = 'Seasonal contracts are billed on the contract, not by the push. Their pushes are counted here for your records.'
...
export function toCsv (report) {
  ...
  for (const r of report.rows) {
    lines.push([
      textCell(r.name), textCell(r.address), textCell(r.billing_label), r.pushes, textCell(r.dates.join('; ')),
      money(r.price_cents), money(r.amount_cents), money(r.hst_cents), money(r.total_cents)
    ].join(','))
  }
