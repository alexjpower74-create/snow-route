...
const PORT = '7606'
// No machine paths in the committed log (clarification 48): the repository roots (this worktree, and the checkout that node_modules
// resolves to) and their file:// forms become <repo>; the home folder becomes <home> as a last guard.
const ROOTS = [
  ...new Set([
    path.resolve(APP, '..'),
    realpathSync(path.resolve(APP, '..')),
    path.resolve(realpathSync(path.join(APP, 'node_modules')), '..', '..'),
  ]),
...
const summary = (out) =>
  out
    .split('\n')
    .filter((l) => /^\s*(\d+ (passed|failed|skipped|flaky)|✘|✓)/.test(l))
    .join('\n')
const redLines = (out) => {
...
export function control({ name, what, args, breakIt }) {
  ...
  if (clean.code !== 0) {
    appendFileSync(
      LOG,
      scrub(
        `== ${stamp} ${name}: VOID. The unbroken copy did not pass, so a red run would prove nothing.\n${summary(clean.out)}\n${redLines(clean.out)}\n\n`,
      ),
    )
